...
/**
 * The base class for map pane cursor tools. Simply adds a getCursor method to
 * the MapToolAdapter
 * 
...
public abstract class CursorTool extends MapMouseAdapter {
	...
	/**
	 * Flag indicating that the tool should be triggered whenever any mouse
	 * button is used.
	 */
	public static final int ANY_BUTTON = SWT.BUTTON_MASK;

	private MapControl mapPane;
	private int triggerButtonMask;

	/**
	 * Constructs a new cursor tool. To activate the tool only on certain mouse
	 ...
	 *             if mapPane is null
	 */
	public void setMapPane(MapControl pane) {
		if (pane == null) {
	...
	/**
	 * Get the cursor for this tool. Sub-classes should override this method to
	 * provide a custom cursor.
	 *
	...
	/**
	 * Returns <code>true</code> for any tool which is drawing while dragging.
	 * For tools which are triggered only by a certain mouse event it might be
	 * the case that {@link #canDraw()} is <code>true</code> while they are
	 * actually not active.
	 *
	...
	protected boolean isTriggerMouseButton(MapMouseEvent event) {
		return triggerButtonMask == ANY_BUTTON || // on mouse move or mouse drag
													// the mouse button field is
													// 0 but the state mask is
													// set
				0 != (triggerButtonMask & event.getStateMask()) || // on mouse
																	// click the
																	// state
																	// mask is
																	// 0, but
